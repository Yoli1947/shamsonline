...
// --- PRODUCTOS ---
export async function getAllProducts(page = 1, limit = 10, search = '', customOffset = null, isStorefront = false) {
    ...
    const columns = isStorefront
        ? `id, name, description, price, sale_price, gender, image_url, features, is_published, is_active, brand:brands(id, name), category:categories(id, name), images:product_images(url, is_primary, sort_order), variants:product_variants(id, size, stock)`
        : `*, brand:brands(id, name), category:categories(id, name), images:product_images(id, url, is_primary, alt_text, sort_order), variants:product_variants(id, size, color, stock, sku)`;
...
export async function getBrands() {
    ...
    let { data, error } = await baseQuery().order('sort_order', { ascending: true }).order('name', { ascending: true })
...
export async function createBrand(brandData) {
    ...
    if (upp.includes('PERRAMUS') && (upp.includes('INTEX') || upp.includes('INDITEX'))) {
        name = 'PERRAMUS';
    }

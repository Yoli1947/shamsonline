...
export async function getProductBySku(sku) {
    const { data, error } = await supabase
        .from('products')
        .select(`id, sku, name, description, price, sale_price, gender, image_url, features, is_published, is_active, sort_order, is_featured,
...
export async function getAllProducts(page = 1, limit = 10, search = '', customOffset = null, isStorefront = false, syncStamp = null) {
    ...
    const columns = isStorefront
        ? `id, sku, name, description, price, sale_price, gender, image_url, features, is_published, is_active, is_featured, sort_order, brand:brands!inner(id, name, card_image_url), category:categories(id, name), images:product_images(url, is_primary, sort_order), variants:product_variants(id, size, stock)`

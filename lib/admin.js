...
export async function getAllProducts(page = 1, limit = 10, search = '', customOffset = null, isStorefront = false, syncStamp = null) {
    ...
    const columns = isStorefront
        ? `id, name, description, price, sale_price, gender, image_url, features, is_published, is_active, brand:brands(id, name, card_image_url), category:categories(id, name), images:product_images(url, is_primary, sort_order), variants:product_variants(id, size, stock)`
...
export async function getBrands(syncStamp = null) {
    let query = supabase
        .from('brands')
        .select(`
            *,
...
export async function getCategories(syncStamp = null) {
    let query = supabase
        .from('categories')
        .select('*')
        .eq('is_active', true)

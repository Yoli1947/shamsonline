...
// La columna has_defect es reciente (migración 20260819); mientras no se haya
// aplicado en la DB, cualquier select que la incluya devuelve error. Reintentamos
// sin ella para no romper la carga del catálogo si la migración todavía no corrió.
function isMissingColumnError(error) {
    return !!error && /has_defect/i.test(error.message || '') && /column .* does not exist/i.test(error.message || '')
}

export async function getProductBySku(sku) {
    const runQuery = (includeDefect) => supabase
        .from('products')
        .select(`id, sku, name, description, price, compare_at_price, sale_price, gender, image_url, features, is_published, is_active, sort_order, is_featured, is_sale_pick, is_anticipo_ss27,
            brand:brands(id, name, card_image_url, is_active),
            category:categories(id, name),
            images:product_images(url, is_primary, sort_order),
            variants:product_variants(id, size, stock${includeDefect ? ', has_defect' : ''})`)
        .eq('sku', sku)
    ...
    if (data?.brand?.is_active === false) return null
    return data
}
...
export async function getAllProducts(page = 1, limit = 10, search = '', customOffset = null, isStorefront = false, syncStamp = null) {
    ...
    const buildQuery = (includeDefect) => {
        // Si es para la tienda (storefront), pedimos menos columnas para ir más rápido
        // En storefront usamos !inner en brand para poder excluir marcas ocultas (is_active = false) a nivel de DB
        const columns = isStorefront
            ? `id, sku, name, description, price, compare_at_price, sale_price, gender, image_url, features, is_published, is_active, is_featured, is_sale_pick, is_anticipo_ss27, sort_order, created_at, season, brand:brands!inner(id, name, card_image_url), category:categories(id, name), images:product_images(url, is_primary, sort_order), variants:product_variants(id, size, stock${includeDefect ? ', has_defect' : ''})`
            : `*, brand:brands(id, name, card_image_url), category:categories(id, name), images:product_images(id, url, is_primary, alt_text, sort_order), variants:product_variants(id, size, color, stock, sku${includeDefect ? ', has_defect' : ''})`;

        let query = supabase
            .from('products')
            .select(columns, isStorefront ? undefined : { count: 'exact' })

        // CACHE-BUSTING: Agregamos un filtro que siempre es verdadero pero cambia la URL
        if (syncStamp) {
            query = query.neq('id', '00000000-0000-0000-0000-' + (syncStamp % 1000000000000).toString().padStart(12, '0'))
        }

        if (isStorefront) {
            query = query.eq('is_published', true).or('is_active.eq.true,is_active.is.null')
            query = query.not('name', 'ilike', '%prueba%').not('name', 'ilike', '%test%')
            // Excluir productos cuya marca fue ocultada desde el admin
            query = query.eq('brand.is_active', true)
        }
        ...
        query = query.order('created_at', { ascending: false })
            // Desempate estable: muchos productos comparten el mismo created_at
            // exacto (lotes del sistema de ventas), y sin un desempate único la
            // paginación por rango no es determinística — algunos productos
            // aparecen duplicados en varias páginas y otros quedan afuera.
...
export async function createProduct(productData) {
    const { data, error } = await supabase
        ...
        .insert({
            ...
            is_sale_pick: productData.isSalePick ?? false,
            is_anticipo_ss27: productData.isAnticipoSS27 ?? false,
            ...
            sort_order: productData.sortOrder || null
        })
        .select()
        .single()
...
export async function updateProduct(id, productData) {
    ...
    if (productData.isSalePick !== undefined) updateData.is_sale_pick = productData.isSalePick
    if (productData.isAnticipoSS27 !== undefined) updateData.is_anticipo_ss27 = productData.isAnticipoSS27
    ...
    if (productData.sortOrder !== undefined) updateData.sort_order = productData.sortOrder
...
export async function updateVariantStock(variantId, newStock) {
    ...
    // Invalidar caché de la tienda: sin esto, los navegadores con el catálogo
    // ya cacheado no se enteran de que el stock cambió.
    try { await updateSiteSetting('last_catalog_update', new Date().toISOString()) } catch { }

    return data
}

/**
 * Marcar/desmarcar una variante como "con falla". Las unidades con falla
 * quedan excluidas del stock disponible en la tienda, pero siguen visibles
 * en el admin para poder gestionarlas.
 */
export async function updateVariantDefect(variantId, hasDefect) {
    const { data, error } = await supabase
        .from('product_variants')
        .update({ has_defect: hasDefect })
        .eq('id', variantId)
        .select()
        .single()
    ...
    // Invalidar caché de la tienda (ver comentario en updateVariantStock).
    try { await updateSiteSetting('last_catalog_update', new Date().toISOString()) } catch { }
...
export async function deleteCancelledOrders() {
    ...
    const { error: ordersError } = await supabase
        ...
        .delete()
        .in('id', ids)
    if (ordersError) throw new Error(handleSupabaseError(ordersError))
...
/**
 * Eliminar un artículo de un pedido y recalcular totales
 */
export async function removeOrderItem(orderId, itemId) {
    // 1. Obtener datos del item a eliminar
    const { data: item, error: itemError } = await supabase
        .from('order_items')
        .select('id, variant_id, quantity, subtotal')
        .eq('id', itemId)
        .single()
    if (itemError) throw new Error(handleSupabaseError(itemError))

    // 2. Verificar que no sea el único item
    const { count } = await supabase
        .from('order_items')
        .select('id', { count: 'exact', head: true })
        .eq('order_id', orderId)
    if (count <= 1) throw new Error('No se puede eliminar el único artículo del pedido. Anulá el pedido completo.')

    // 3. Restaurar stock si tiene variante
    if (item.variant_id) {
    ...
    // 5. Recalcular subtotal y total del pedido
    const { data: remaining } = await supabase
        .from('order_items')
        .select('subtotal')
        .eq('order_id', orderId)
    const newSubtotal = (remaining || []).reduce((s, r) => s + Number(r.subtotal), 0)

    const { data: order } = await supabase.from('orders').select('discount, shipping_cost').eq('id', orderId).single()
    const newTotal = newSubtotal - Number(order?.discount || 0) + Number(order?.shipping_cost || 0)
...
// Temporadas realmente en uso en el catálogo (viene del sistema de ventas,
// que puede introducir una temporada nueva en cualquier sincronización sin
// avisar). Se combina con la lista manual de abajo para que "Gestión de
// Temporadas" nunca quede desactualizada ni oculte productos por un filtro
// que no incluye su temporada real.
async function getSeasonUsageCounts() {
    const counts = {}
    const CHUNK = 1000
    let offset = 0
    let hasMore = true
    while (hasMore) {
        ...
        for (const row of data || []) {
            const s = row.season && row.season.trim()
...
async function getSeasonsInUse() {
    return Object.keys(await getSeasonUsageCounts())
}

// Igual que getSeasons() pero también informa cuántos productos tiene cada
// temporada, para que la UI pueda avisar que una temporada en uso no se
// puede "borrar" de verdad (siempre va a reaparecer mientras haya productos
// con esa temporada).
export async function getSeasonsWithUsage() {
    const manual = await getManualSeasons()

    let usage = {}
    try {
        usage = await getSeasonUsageCounts()
    } catch (e) {
        console.error('Error leyendo temporadas en uso:', e)
    }

    const names = Array.from(new Set([...manual, ...Object.keys(usage)])).sort()
    return names.map(name => ({ name, count: usage[name] || 0 }))
}

async function getManualSeasons() {
    const raw = await getSiteSetting('product_seasons')
    if (!raw) return []
    let list = []
    try { list = JSON.parse(raw) } catch (e) { return [] }
    // "todas" quedó guardada como si fuera una temporada real en algún momento;
    // no es un valor de temporada válido (ningún producto la tiene), es la
    // opción de "sin filtro" — se maneja aparte en la UI, no como chip.
    return list.filter(s => s && s.trim().toLowerCase() !== 'todas')
}
...
export async function getSeasons() {
    ...
    let inUse = []
    try {
        inUse = await getSeasonsInUse()
    } catch (e) {
        console.error('Error leyendo temporadas en uso:', e)
    }

    return Array.from(new Set([...manual, ...inUse])).sort()
}
...
export async function createSeason(name) {
    ...
    if (!manual.includes(name)) {
        await saveSeasons([...manual, name].sort())
    }
...
export async function deleteSeason(name) {
    const manual = await getManualSeasons()
    await saveSeasons(manual.filter(s => s !== name))
    return await getSeasons()
}
...
/**
 * Sincronizar clientes desde pedidos existentes (backfill) — via Edge Function con service role
 */
export async function syncCustomersFromOrders() {
    const { data: { session } } = await supabase.auth.getSession()
    const token = session?.access_token || import.meta.env.VITE_SUPABASE_ANON_KEY
    const res = await fetch(`${import.meta.env.VITE_SUPABASE_URL}/functions/v1/sync-customers`, {
        method: 'POST',
        headers: { 'Authorization': `Bearer ${token}`, 'Content-Type': 'application/json' }
    })
    const data = await res.json()
    if (!res.ok || data?.error) throw new Error(data?.error || 'Error al sincronizar')
...
export async function getAllProductsForOrdering() {
    ...
    while (hasMore) {
        const { data, error, count } = await supabase
            .from('products')
            .select('id, name, sort_order, season, is_active, is_published, is_featured, is_sale_pick, is_anticipo_ss27, images:product_images(url, is_primary), brand:brands(name)', { count: 'exact' })
            .eq('is_published', true)
            .eq('is_active', true)

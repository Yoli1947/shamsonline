...
export async function getAllProductsForOrdering() {
    ...
    while (hasMore) {
        const { data, error, count } = await supabase
            .from('products')
            .select('id, name, sort_order, is_active, is_published, is_featured, is_sale_pick, is_anticipo_ss27, images:product_images(url, is_primary), brand:brands(name)', { count: 'exact' })
            .eq('is_published', true)
            .eq('is_active', true)

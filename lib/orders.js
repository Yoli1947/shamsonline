...
/**
 * Valida stock en vivo contra la DB para los items de un pedido, justo antes de crearlo.
 * Es el último punto de control: aunque la vitrina haya mostrado datos desactualizados
 * (o el cliente los haya manipulado), acá se rechaza el pedido si algo ya no está disponible.
 * Tolera que la columna has_defect todavía no exista (migración pendiente).
 */
async function assertStockAvailable(items) {
    const variantIds = [...new Set(items.map(i => i.variantId).filter(Boolean))]
    if (variantIds.length === 0) return

    let { data: liveVariants, error } = await supabase
        .from('product_variants')
        .select('id, stock, has_defect')
        .in('id', variantIds)

    if (error) {
        // Columna has_defect todavía no existe en la DB: reintentar solo con stock.
        ;({ data: liveVariants, error } = await supabase
            .from('product_variants')
            .select('id, stock')
            .in('id', variantIds))
    }
    ...
    const byId = Object.fromEntries((liveVariants || []).map(v => [v.id, v]))
    for (const item of items) {
        if (!item.variantId) continue
        const live = byId[item.variantId]
        const available = live && !live.has_defect ? (live.stock || 0) : 0
        if (available < item.quantity) {
            throw new Error(`"${item.name}" (talle ${item.size}) ya no tiene stock disponible. Actualizá la página e intentá de nuevo.`)
        }
    }
}
...
export async function createOrder(orderData, items) {
    ...
    const discountedSubtotal = Math.round(subtotal * (hasCashDiscount ? 0.85 : 1));
    ...
    // Guardar/actualizar cliente en la tabla customers (esperamos el resultado)
    let customerPromise = Promise.resolve();
    if (orderData.email) {
        customerPromise = supabase.from('customers').upsert({
    ...
    // Enviar email de confirmación y esperar a que ambas promesas terminen
    try {
        await Promise.all([
            customerPromise,
        ...
        console.error("Error en tareas post-pedido:", err);
        // No lanzamos error para no romper la experiencia del usuario si solo falló el email
    }
